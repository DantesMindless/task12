...
try:
    class Fraction():
        ...
        def calc(self, num1):
            ...
            self.res1 = round(int(num1.split('/')[0]) / int(num1.split('/')[1]), 2)
            return self.res1


    n1 = input('Enter first fraction x/y')
    op = input('Select operation +,-,*,/')
    n2 = input('Enter first fraction x/y')

    x = Fraction()
    y = Fraction()
    if op == '+':
        ctr = str(round(x.calc(n1) + y.calc(n2), 2))
        print(ctr)
    if op == '-':
        ctr = str(round(x.calc(n1) - y.calc(n2), 2))
        print(ctr)
